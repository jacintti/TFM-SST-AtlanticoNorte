...
// ============================================================================
// SCRIPT 07c2: HYCOM — TENDENCIAS DE TEMPERATURA EN 500 m Y 1000 m
// ----------------------------------------------------------------------------
// PROPÓSITO:
//   Complementar el análisis de temperatura subsuperficial (script 07c1)
//   con los niveles profundos: 500 m y 1000 m. Completar la fotografía
//   vertical de la temperatura del Atlántico Norte en 1993-2023.
//
// HIPÓTESIS:
//   Si la convección profunda del LAB se está debilitando, esperamos:
//     · LAB 500-1000 m: aguas profundas aisladas → poco cambio térmico o
//       posible enfriamiento por persistencia de masas frías históricas.
//     · NOR 500-1000 m: atlantificación llegando a niveles intermedios →
//       calentamiento significativo en línea con lo observado a 100 m.
//     · CBL 500-1000 m: aporte lateral de aguas frías del LAB →
//       posible enfriamiento o estabilidad.
//
// SALIDA:
//   · Tabla con 14 filas (7 regiones × 2 niveles profundos) con tendencia,
//     tau y p-valor de la temperatura.
// ============================================================================
...
var TEND_ANIO_INICIO = 1993;
var TEND_ANIO_FIN    = 2023;

// Para la máscara térmica necesitamos la temperatura de superficie
var BANDA_TEMP_REF = 'water_temp_0';
var BANDAS_TEMP = ['water_temp_500', 'water_temp_1000'];


// ----------------------------------------------------------------------------
// 3. CARGAR Y PREPROCESAR HYCOM
// ----------------------------------------------------------------------------
var hycom_raw = ee.ImageCollection('HYCOM/sea_temp_salinity')
  .filterDate(FECHA_INICIO, FECHA_FIN)
  .filterBounds(geom.AN_limpio)
  .select([BANDA_TEMP_REF].concat(BANDAS_TEMP));
...
var hycom_diario = hycom_raw.map(function(img) {
  ...
  var temp = img.select(BANDAS_TEMP).multiply(0.001).add(20).rename(BANDAS_TEMP);

  // Máscara térmica sobre temperatura superficial
  var mascara = temp_sup.gte(SST_MIN).and(temp_sup.lte(SST_MAX));

  return temp.updateMask(mascara)
             .clip(geom.AN_limpio)
...
var feats_500m = regiones_lista.map(function(r) {
  return calcular_tendencia(r.nombre, r.geometria, 'water_temp_500', '500 m');
});

var feats_1000m = regiones_lista.map(function(r) {
  return calcular_tendencia(r.nombre, r.geometria, 'water_temp_1000', '1000 m');
});

var resultados = ee.FeatureCollection(feats_500m.concat(feats_1000m));
...
// ----------------------------------------------------------------------------
print('==================================================================');
print('SCRIPT 07c2 — TENDENCIAS DE TEMPERATURA HYCOM (500 m y 1000 m)');
print('==================================================================');
print('Periodo:', TEND_ANIO_INICIO, 'a', TEND_ANIO_FIN, '(31 años)');
print('Niveles analizados: 500 m y 1000 m (profundo)');
print('Unidades: °C/década');
print('');
print('TABLA DE TENDENCIAS DE TEMPERATURA HYCOM PROFUNDA:');
print(resultados);